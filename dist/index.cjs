...
class TwilioCoreVendor extends EventEmitter {
    constructor(globalVendorArgs) {
        ...
        this.handlerLocalMedia = (req, res) => {
            ...
            const decryptPath = bot.utils.decryptData(file);
            const decodeFile = decodeURIComponent(decryptPath);
            if (!node_fs.existsSync(decodeFile))
                return res.end(`not exits: ${decodeFile}`);
            const fileStream = node_fs.createReadStream(decodeFile);
    ...
    constructor(args) {
        ...
        this.saveFile = async (ctx, options) => {
            try {
                ...
                const pathFile = require$$1.join(options?.path ?? os.tmpdir());
